...
def display_results(model_radio, input_text, start_date, end_date, summarize_articles, num_matches):
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        summary_checkbox = gr.Checkbox(label="Summarize articles (might take time to load)", scale=2)
        num_matches = gr.Dropdown(choices=[5, 10, 15, 25, 50], value=5, interactive=True, label="Number of articles", scale=2)
    with gr.Row():
    ...
    submit_button.click(fn=display_results, inputs=[model_radio, text_input, start_date[0], end_date[0], summary_checkbox, num_matches], outputs=output_html)
